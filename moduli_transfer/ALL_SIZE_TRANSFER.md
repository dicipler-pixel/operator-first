...
The existing 119 named Offset Lean statements remain unchanged. The formal three-site theorem and conditional transfer-of-limit theorem remain accurately scoped. The additional `OperatorFirst/LaurentBoundary.lean` module supplies general arbitrary-size column bounds, faithful leading-coefficient descent, and the affine-polynomial conclusion under explicit representation hypotheses. Its exact verification is recorded in the accompanying evidence. The model-specific invariant-ring reduction, explicit block similarity, and identification of the actual chart with those general hypotheses still need Lean encoding, as do the physical Fourier construction and external Hankel asymptotics. See `laurent_boundary/FORMAL_BOUNDARY_SCOPE.md`.
