#!/usr/bin/env python3
"""Complete SU(3) theta-graph electric cutoff C<=40/3 (19 states), exact Haar.
...
six_u=((F(1,2),(U,U)),(F(1,2),(U+U,)))
six_v=((F(1,2),(V,V)),(F(1,2),(V+V,)))
BASIS=BASIS+(six_u,pc(six_u),six_v,pc(six_v))
ENERGY=ENERGY+(F(40,3),)*4
...
NAMES=NAMES+('chi6(U)','chibar6(U)','chi6(V)','chibar6(V)')
GRAM_DIAG=GRAM_DIAG+(F(1),)*4
TAIL=F(43,3)
...
def main():
    ...
    ap.add_argument('--targets',type=Path,default=Path(__file__).with_name('two_plaquette_19_targets.json'))
    ap.add_argument('--out',type=Path,default=Path('two_plaquette_19_results.json'))
    ...
    payload=dict(model='Two SU(3) squares sharing one link; alpha=1',
                 cutoff='40/3',dimension=19,omitted_electric_floor=str(TAIL),
                 ...
                 total_pivot_signs=sum(57+len(row['inner_pivots']) for row in rows),
