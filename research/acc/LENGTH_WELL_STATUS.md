...
Therefore the minimum escape ceiling is **exactly 27** and the length-well depth is **exactly +6**.
...
The correct interpretation is qualitative: the hard controls exhibit a genuine uphill barrier that is absent from the accepted controls tested so far. Well depth is **not** currently justified as a numerical estimator of solution length.
...
The official/public length-first baseline is structurally blind to states that must get longer before cancellation becomes available. The length-well eye measures exactly that obstruction instead of penalizing it.
