...
with χ_p the Legendre symbol. Averaging over many primes gives a rank estimate with a computable standard error. **Script `DIO-NAGAO-01`**, primes 401…1499 (161 primes).
...
**j = 0 caveat** [T]. For the b = 0 equations the fibres have j = 0, so a_p = 0 at p ≡ 2 mod 3 and those primes carry no rank information — they read exactly 0.000 and must be dropped, which doubles the error bar on those five surfaces. This is handled explicitly in the script.
...
so **one pass per branch pair scans the entire infinite family of twist classes c**, because c enters only through the character χ_c(p). Shioda–Tate caps the sum of twist ranks over all c at 2 for branch points off the singular fibres, so at most two c per pair can spike; a forest of small bumps kills the whole pair.
