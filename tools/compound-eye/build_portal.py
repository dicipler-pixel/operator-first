...
elemental=ROOT/'projects/elemental_peel/Elemental_Peel_Observatory.html'
if elemental.exists():
    page=page.replace('<button data-tab="integration">','<button data-tab="elemental">Elemental Peel</button><button data-tab="integration">',1)
    panel='<section id="elemental" hidden><h2>Elemental spacing, channels and the surface</h2><iframe title="Elemental Peel Observatory" style="width:100%;height:1900px;border:0" sandbox="allow-scripts" srcdoc="'+html_module.escape(elemental.read_text(),quote=True)+'"></iframe></section>'
    page=page.replace('</main>',panel+'</main>',1)
