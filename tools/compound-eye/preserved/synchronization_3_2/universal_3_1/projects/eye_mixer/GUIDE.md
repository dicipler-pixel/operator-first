...
Universal 3.1 · Eye Mixer 1.0.0 · 7 September 2026
...
The catalog still contains all **178 registered eye versions**. The mixer is a reusable **display layer**, not another scientific theorem and not a claim that every catalog eye has run. The built-in studies supply scalar traces for appropriate views. Any other eye can enter through an exported common-coordinate trace; matrix or tensor outputs first need an explicit observable, component or invariant.
