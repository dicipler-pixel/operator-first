...
**Edition 0.4 — implemented decision DAGs and new exclusions.** Read `dag_search/EXECUTION_REPORT.html` for the current results. The new engine stores scoped obstructions, shares exact relation spaces, and independently checks complete finite coverage. It excludes 470,232 distinct new arithmetic Kakeya configurations and 24,976 Earth–Moon host extensions. Raw baseline search time is unchanged; producing and checking the evidence measured about 35.9 times faster. Both full Epoch targets and the new Lean formalizations remain open. Earlier editions below document the preserved examples and baseline.

Verify the entire delivered proof bundle without third-party dependencies:

```bash
python3 dag_search/check_all.py
```

Run a supplied search request through the updated interface:
...
Complete benchmark and fresh-search commands are in `dag_search/EXECUTION_REPORT.md`. No script editing or GitHub checkout is required.
