...
def main():
    ...
    assert counts['eye_versions'] == len(registry.eyes) == 191
    assert counts['sets'] == len(registry.sets) == 29
    ...
    for element in ['mixer', 'horizon', 'integration', 'arithmetic', 'catalog']:
        assert f'id="{element}"' in page and f'data-tab="{element}"' in page
    assert 'Universal 3.2' in page and '<b>191</b>eye versions' in page
